...
function SaltFlipCard({
  ...
  // On mobile: tile after tile, each flips at 10s, 20s, 30s, 40s; show back for 10s then front again
  const autoFlipped =
    mobileSeconds != null &&
    mobileSeconds >= MOBILE_FLIP_INTERVAL_SEC * (index + 1) &&
    (mobileSeconds - MOBILE_FLIP_INTERVAL_SEC * (index + 1)) % (MOBILE_FLIP_INTERVAL_SEC * 2) < MOBILE_FLIP_INTERVAL_SEC;

  const flipped = isMobile && mobileSeconds != null ? (manualFlipped ?? autoFlipped) : manualFlipped ?? false;
