...
export default function DashboardResourcesClient({
  initialResources,
}: {
  initialResources: ResourceRow[];
}) {
  const [resources, setResources] = useState(initialResources);
  const [resourceType, setResourceType] = useState<keyof typeof RESOURCE_TYPE>("programme");
  ...
  async function handleUpload(e: React.FormEvent<HTMLFormElement>) {
    ...
    const endpoint =
      resourceType === RESOURCE_TYPE.admin
    ...
    try {
      ...
      if (resourceType === RESOURCE_TYPE.programme) {
        setResources((prev) => [
          {
            id: data.id,
        ...
        setUploadSuccess("File uploaded to Admin resources. View it under the Admin Resources tab.");
      }
      form.reset();
      setResourceType("programme");
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <>
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-teal-blue mb-4">Upload resource</h2>
        <form onSubmit={handleUpload} className="space-y-4 max-w-md">
          ...
          {uploadSuccess && (
            <p className="text-green-700 text-sm bg-green-50 px-3 py-2 rounded">{uploadSuccess}</p>
          )}
          <div>
            ...
            <select
              id="resource_type"
              value={resourceType}
              onChange={(e) => setResourceType(e.target.value as keyof typeof RESOURCE_TYPE)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-lime-green focus:border-lime-green"
              required
            >
              <option value={RESOURCE_TYPE.programme}>
                Programme resource (visible to students & teachers)
              </option>
              <option value={RESOURCE_TYPE.admin}>
                Admin only (internal use, stored in Admin Resources)
              </option>
            </select>
            <p className="text-xs text-gray-500 mt-1">
              Programme resources use the Resources bucket; admin-only files use the Admin Resources bucket.
            </p>
          </div>
          <div>
      ...
      <section>
        <h2 className="text-xl font-semibold text-teal-blue mb-4">Current resources</h2>
        {resources.length === 0 ? (
          <p className="text-gray-500">No resources yet. Upload one above.</p>
        ) : (
